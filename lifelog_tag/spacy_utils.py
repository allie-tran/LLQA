...
if __name__ == "__main__":
    ...
    sent = "Peter wakes up in his bedroom."
    doc = nlp(sent)
    print([token.text for token in doc])
    print(list(srl.get_all_tags(doc)))
    root = -1
    root = [token for token in doc if token.head == token]
    assert root, "can't find root"
    root = doc[2]
    for d in display_tree([root.i], root, -1):
        print(" ".join([token.text for token in doc if token.i in d]))
