...
def get_max_epoch_model(output_dir):
    fn_model_list = glob.glob(os.path.join(
        output_dir, "ckpt-*/pytorch_model.bin"))
    ...
    both_set = set([int(os.path.basename(fn).split('.')[1]) for fn in fn_optim_list])
    if both_set:
        return max(both_set)
    else:
        return None

class WeightedDistributedSampler(torch.utils.data.DistributedSampler):
...
def load_and_cache_examples(
    ...
    if cached_features_file is not None and os.path.exists(cached_features_file):
        ...
        for example in tqdm.tqdm(examples):
            ...
            targets.append(example["tgt"].lower())

        if shuffle:
            logger.info("Shuffling, re-indexing, suffering...")
            count = Counter(targets)
            high_features = [i for i in range(len(targets)) if count[targets[i]] >= 20]
            low_features = [i for i in range(
                len(targets)) if count[targets[i]] < 20]

            high_weights = torch.DoubleTensor(
                [1./np.log(count[targets[i]] + 1) for i in high_features])
            low_weights = torch.DoubleTensor(
                [1./np.log(count[targets[i]] + 1) for i in low_features])

            # targets = np.array(targets)
            # unique = list(np.unique(targets))
            # class_sample_count = np.array(
            #     [np.log(len(np.where(targets == t)[0]) + 1) for t in unique])
            # weight = 1. / class_sample_count
            # samples_weight = np.array([weight[unique.index(t)] for t in targets])
            # samples_weight = torch.from_numpy(samples_weight)


            low_indices = []
            split_size = len(low_weights) // 5
            for i in range(5):
                batch_weights = low_weights[split_size * i: split_size * (i + 1)]
                batch_indices = [id + split_size * i for id in torch.multinomial(batch_weights, len(
                    batch_weights), replacement=False).tolist()]
                if len(batch_indices) < split_size:
                    batch_indices += random.choices(range(0, len(low_weights)),
                                                    weights=low_weights, k=split_size - len(batch_indices))
                low_indices.append(batch_indices)
            high_indices = torch.multinomial(
                high_weights, split_size, replacement=False).tolist()
            new_features = []
            for i in range(len(high_indices)):
                new_features.extend([features[high_indices[i]]] + [features[low_indices[j][i]] for j in range(5)])
